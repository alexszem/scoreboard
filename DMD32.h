...
class DMD
{
  public:
    //Instantiate the DMD
    DMD(byte panelsWide, byte panelsHigh, bool rotate);
	//virtual ~DMD();
  ...
  private:
    ...
    inline uint16_t logicalWidth() const {
        return rotate ? (DMD_PIXELS_DOWN * DisplaysHigh)
                      : (DMD_PIXELS_ACROSS * DisplaysWide);
    }

    inline uint16_t logicalHeight() const {
        return rotate ? (DMD_PIXELS_ACROSS * DisplaysWide)
