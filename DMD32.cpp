...
--------------------------------------------------------------------------------------*/
DMD::DMD(byte panelsWide, byte panelsHigh, bool rotate)
{
    uint16_t ui;
...
void DMD::writePixel(unsigned int bX, unsigned int bY, byte bGraphicsMode, byte bPixel)
{
    ...
    if (bX >= logicalWidth() || bY >= logicalHeight()) {
        return;
    }

    if (rotate) {
        unsigned int oldX = bX;
        unsigned int oldY = bY;

        // 90 degrees clockwise
        bX = oldY;
        bY = (DMD_PIXELS_DOWN * DisplaysHigh - 1) - oldX;
    }

    // bounds check in physical space
		     ...
		     byte bGraphicsMode)
{
    ...
    for (int i = 0; i < length; i++) {
        ...
        if ((bX + strWidth) >= logicalWidth() || bY >= logicalHeight()) return;
    }
}
...
boolean DMD::stepMarquee(int amountX, int amountY)
{
    ...
    if (marqueeOffsetX < -marqueeWidth) {
	    marqueeOffsetX = logicalWidth();
	    clearScreen(true);
        ret=true;
    } else if (marqueeOffsetX > logicalWidth()) {
    ...
    if (marqueeOffsetY < -marqueeHeight) {
	    marqueeOffsetY = logicalHeight();
	    clearScreen(true);
        ret=true;
    } else if (marqueeOffsetY > logicalHeight()) {
   ...
   if( digitalRead( PIN_OTHER_SPI_nCS ) == HIGH )
   {
	...
	vspi->beginTransaction(SPISettings(spiClk, MSBFIRST, SPI_MODE0));
        for (int i=0;i<rowsize;i++) {
            ...
            uint8_t b0 = bDMDScreenRAM[offset + i];
            uint8_t b1 = bDMDScreenRAM[offset + i + row1];
            uint8_t b2 = bDMDScreenRAM[offset + i + row2];
            uint8_t b3 = bDMDScreenRAM[offset + i + row3];
...
{
    if (bX >= logicalWidth() || bY >= logicalHeight()) return -1;
...
void DMD::setInvert(bool invert)
{
    this->invert = invert;
}

bool DMD::getInvert() const
{
    return invert;
}
